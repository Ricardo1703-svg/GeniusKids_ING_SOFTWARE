...
class Intermedio : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_main_inter_lenguaje)

        regre = findViewById(R.id.button3)
        ...
        // Verificar si el progreso de "Fácil" está completo
    ...
    private fun verificarProgresoFacil() {
        ...
        userDocRef.get()
            .addOnSuccessListener { document ->
                if (document.exists()) {
                    val progressFacil = document.getLong("Lenguaje.Facil") ?: 0
                    val totalVideosFacil = 5// Puedes obtener este valor dinámicamente si es necesario

                    if (progressFacil >= totalVideosFacil) {
                        cargarVideos("Intermedio", "Lenguaje")
                    } else {
                        Toast.makeText(this, "Debes completar la dificultad 'Fácil' antes de acceder", Toast.LENGTH_SHORT).show()
                        layoutVideos.removeAllViews() // Asegúrate de no mostrar ningún video
                    }
                }
            }
    ...
    private fun displayVideos(contenidos: List<Video>, actividades: List<Video>) {
        ...
        for (i in 0 until maxCount) {
            val rowView = LayoutInflater.from(this).inflate(R.layout.row_buttons_lenguaje, layoutVideos, false)
    ...
    private fun openWebView(videoUrl: String) {
        val intent = Intent(this, WebViewTosLenguaIntermedio::class.java)
